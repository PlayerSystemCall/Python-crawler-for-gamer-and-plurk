...
def go_to_web(web_URL): #測試網路連結的狀態
    import time    
    web_number = 0 #程式調整的網路狀態碼
    web_testtime = 0 #測試網路連線的次數
    while web_number != 200 or web_testtime != 3: #當網路狀態碼等於200或測試次數達到3次時，結束迴圈
        ...
        if web_number == 200 or web_testtime == 3: #如果網路正常或是測試網路連線3次
            return web_number #回傳程式調整的網路狀態碼
        else:
            return web_number #回傳程式調整的網路狀態碼

def get_followday(number, start_time, i, state, startfollowdate, followday, string, endfollowdate): #取得追蹤天數
    if state == "old": #如果狀態為old(老追蹤者)
        try:
            ...
            if str(start_time).split(" ")[0] == str(open_googlesheets.worksheet_by_title("噗浪追蹤名單").get_value("E{}".format(number+2))): #start_time的日期等於("D{}".format(number+1))的字串
...
try:    
    ...
    code_section_1_state="〇"
except:
    code_section_1_state="✕"

try:
    #取得巴哈小屋網頁原始碼，找到資料的的html區塊，取出資料
    gamer_url = "https://home.gamer.com.tw/homeindex.php?owner=player0309" #巴哈小屋網址
    ...
    if gamer_statuscode == 200: #如果網路正常
        ...
        for i in gamer_dates_on_jsxml: #將7個日期全部由字串轉成datetime格式
            gamer_dates_on_jsxml[down] = datetime.strptime(i, "%Y/%m/%d")
            down = down+1
        #小屋/小屋統計/好友圈人數
    ...
    code_section_2_state="〇"
except:
    code_section_2_state="✕"
...
try:
    ...
    gamer_followerlist_url = "https://home.gamer.com.tw/friendList.php?user=player0309&t=4" #巴哈追蹤者列表網址
    ...
    for i in range(len(gamer_data_get["other"]["gamer_follower_accountlist"])): #依照順序數字建立字典，再將資訊分別塞進去
        ...
        gamer_follower_url = "https://home.gamer.com.tw/homeindex.php?owner={}".format(gamer_data_get["follower"][i]["account"]) #組合成追蹤者的巴哈小屋(舊版)連結
        gamer_follower_statuscode = go_to_web(gamer_follower_url) #回傳網路狀態碼
        if gamer_follower_statuscode != 200: #如果網路不正常
            gamer_follower_url = "https://home.gamer.com.tw/profile/index.php?owner={}".format(gamer_data_get["follower"][i]["account"]) #組合成追蹤者的巴哈小屋(新版)連結
            gamer_follower_statuscode = go_to_web(gamer_follower_url) #回傳網路狀態碼
            OorN = "New" #標示為新版小屋
        else:
            OorN = "Old" #標示為舊版小屋
        ...
        down = 0 #表示狀態
        if gamer_follower_statuscode == 200 and OorN == "Old": #如果可以用舊版小屋連線
            ...
            while down == 0 and gamer_title_number < len(gamer_title): #當down等於0和gamer_title_number小於巴哈大標數量時，持續運作
        ...
        elif gamer_follower_statuscode == 200 and OorN == "New": #如果可以用新版小屋連線，目前程式還不支援去新版尋找
    ...
    code_section_3_state="〇"
except:
    code_section_3_state="✕"

try:
    #取得巴哈朋友名單網頁原始碼，找到資料的的html區塊，取出資料
    gamer_friendlist_url = "https://home.gamer.com.tw/friendList.php?user=player0309&t=1" #巴哈好友列表網址
    ...
    for i in range(len(gamer_data_get["other"]["gamer_friend_accountlist"])): #依照順序數字建立字典，再將資訊分別塞進去
        ...
        if gamer_friend_statuscode != 200: #如果網路不正常
            gamer_friend_url = "https://home.gamer.com.tw/profile/index.php?owner={}".format(gamer_data_get["friend"][i]["account"]) #組合成追蹤者的巴哈小屋(新版)連結
            gamer_friend_statuscode = go_to_web(gamer_friend_url) #回傳網路狀態碼
            OorN = "New" #標示為新版小屋
        else:
            OorN = "Old" #標示為舊版小屋
        ...
        gamer_title_number = 0 #巴哈小屋大標的序號
        down = 0 #表示狀態
        if gamer_friend_statuscode == 200 and OorN == "Old": #如果可以用舊版小屋連線
            ...
            while down == 0 and gamer_title_number < len(gamer_title): #當down等於0和gamer_title_number小於巴哈大標數量時，持續運作
                if "個人紀錄" == gamer_title[gamer_title_number]:
                    ...
                    gamer_title_number = gamer_title_number+1 #巴哈小屋大標的序號加1
            if down == 0: #巴哈姆特模塊化特色所致
                gamer_data_get["friend"][i]["regdate"] = "舊版無區塊"
                gamer_data_get["friend"][i]["lastondate"] = "舊版無區塊"
        elif gamer_friend_statuscode == 200 and OorN == "New": #如果可以用新版小屋連線，目前程式還不支援去新版尋找
    ...
    code_section_4_state="〇"
except:
    code_section_4_state="✕"

try:
    #開啟試算表
    certificate = pygsheets.authorize(service_file='.\pelagic-radio-361715.json') #取得位置在同層級目錄的Google sheets API憑證
    googlesheets_url = "https://docs.google.com/spreadsheets/d/1vLopfsKHRNaS02bI5AmKHsBbbqtL4EbY4k47SRivMSY" #有google sheets ID的網址
    open_googlesheets = certificate.open_by_url(googlesheets_url) #開啟Google sheets
    open_googlesheets_state = 1 #等於True
except:
    open_googlesheets_state = 0 #等於False

if open_googlesheets_state == 1:
    try:
        #獲取運作天數
        worksheet = open_googlesheets.worksheet('id', 1888051586) #定位試算表位置為倒數第2張的"系統訊息"
        ...
        basic_state = 1 #等於True
    except:
        basic_state = 0 #等於False
    
    if basic_state == 1:
        try:
            #寫入試算表1："人氣紀錄"
            worksheet = open_googlesheets.worksheet('id', 0) #定位試算表位置為第1張的"人氣紀錄表"
            
            writesit = 3
            while worksheet.get_value("A{}".format(writesit)) != "":
                writesit = writesit+1
            
            if str(datetime.strptime(worksheet.get_value("A{}".format(writesit-1))+worksheet.get_value("C{}".format(writesit-1)), "%Y年%m月%d日")) == str(datetime.strptime(ad_year_yesterday+date_yesterday, "%Y年%m月%d日")):
                gamer_writesit = writesit-1 #巴哈資料被寫下的格數
            elif str(datetime.strptime(worksheet.get_value("A{}".format(writesit-1))+worksheet.get_value("C{}".format(writesit-1)), "%Y年%m月%d日")) == str(datetime.strptime(ad_year_today+date_today, "%Y年%m月%d日")):
            ...
            gamer_yesterday_allview = "=394+SUM($F$3:F{})".format(gamer_writesit) #從指定格取得前一天的巴哈總人氣數 #計算今天的巴哈總人氣數
            if worksheet.get_value("A{}".format(writesit-1))+worksheet.get_value("C{}".format(writesit-1)) != ad_year_today+date_today:
            ...
            code_section_5_state="〇"
        except:
            code_section_5_state="✕"
        
        try:
            #寫入試算表2："巴哈追蹤名單"
            worksheet = open_googlesheets.worksheet('id', 1960934836) #定位試算表位置為"巴哈姆特追蹤名單"
            ...
            for i in range(len(gamer_data_get["follower"])): #範圍為follower鍵的值(字典)內所含有的數量
                ...
                try:
                    if gamer_data_get["follower"][i]["account"] in gamer_data_last["other"]["gamer_follower_accountlist"]: #如果從巴哈取得的追蹤者account的值有在串列裡
                        gamer_data_new["follower"][gamer_follower_number_new]["state"] = "old" #標記狀態為old
                        ...
                        gamer_data_new["follower"][gamer_follower_number_new]["followday"] = get_followday(number, start_time, i, gamer_data_new["follower"][gamer_follower_number_new]["state"], gamer_data_new["follower"][gamer_follower_number_new]["startfollowdate"], gamer_data_last["follower"][gamer_data_last["other"]["gamer_follower_accountlist"].index(gamer_data_new["follower"][gamer_follower_number_new]["account"])]["followday"], gamer_follower_string, "0") #傳入副程式start_time, i, state, startfollowdate, followday, string, endfollowdate這些參數，取得追蹤天數
            ...
            for i in range(len(gamer_data_get["follower"])): #範圍為follower鍵的值(字典)內所含有的數量
                ...
                try:
                    if gamer_data_get["follower"][i]["account"] not in gamer_data_last["other"]["gamer_follower_accountlist"]: #如果從巴哈取得的追蹤者account的值沒有在串列裡
                        gamer_data_new["follower"][gamer_follower_number_new]["state"] = "new" #標記狀態為new
            ...
            for i in range(len(gamer_data_get["follower"])): #範圍為follower鍵的值(字典)內所含有的數量
                ...
                try:
                    if gamer_data_last["follower"][i]["account"] not in gamer_data_get["other"]["gamer_follower_accountlist"]: #如果從試算表取得的追蹤者account的值沒有在串列裡
                        gamer_data_new["follower"][gamer_follower_number_new]["state"] = "leave" #標記狀態為leave
            ...
            for i in range(len(gamer_data_get["friend"])): #範圍為friend鍵的值(字典)內所含有的數量
                ...
                try:
                    if gamer_data_get["friend"][i]["account"] in gamer_data_last["other"]["gamer_friend_accountlist"]: #如果從巴哈取得的朋友account的值有在串列裡
                        gamer_data_new["friend"][gamer_friend_number_new]["state"] = "old" #標記狀態為old
            ...
            for i in range(len(gamer_data_get["friend"])): #範圍為friend鍵的值(字典)內所含有的數量
                ...
                try:
                    if gamer_data_get["friend"][i]["account"] not in gamer_data_last["other"]["gamer_friend_accountlist"]: #如果從巴哈取得的朋友account的值有不在串列裡
                        gamer_data_new["friend"][gamer_friend_number_new]["state"] = "new" #標記狀態為new
                        gamer_data_new["friend"][gamer_friend_number_new]["account"] = gamer_data_get["friend"][i]["account"] #將從巴哈取得的朋友account、nickname、建立日和最後上線日記錄進gamer_data_new
                        gamer_data_new["friend"][gamer_friend_number_new]["nickname"] = gamer_data_get["friend"][i]["nickname"]
                        gamer_data_new["friend"][gamer_friend_number_new]["regdate"] = gamer_data_get["friend"][i]["regdate"]
                        gamer_data_new["friend"][gamer_friend_number_new]["lastondate"] = gamer_data_get["friend"][i]["lastondate"]
            ...
            for i in range(len(gamer_data_get["friend"])): #範圍為friend鍵的值(字典)內所含有的數量
                ...
                try:
                    if gamer_data_last["friend"][i]["account"] not in gamer_data_get["other"]["gamer_friend_accountlist"]: #如果從試算表取得的朋友account的值沒有在串列裡
                        gamer_data_new["friend"][gamer_friend_number_new]["state"] = "leave" #標記狀態為leave
                        gamer_data_new["friend"][gamer_friend_number_new]["account"] = gamer_data_get["friend"][i]["account"] #將從巴哈取得的朋友account、nickname、建立日和最後上線日記錄進gamer_data_new
                        gamer_data_new["friend"][gamer_friend_number_new]["nickname"] = gamer_data_get["friend"][i]["nickname"]
                        gamer_data_new["friend"][gamer_friend_number_new]["regdate"] = gamer_data_get["friend"][i]["regdate"]
                        gamer_data_new["friend"][gamer_friend_number_new]["lastondate"] = gamer_data_get["friend"][i]["lastondate"]
            ...
            for i in range(3, (max(gamer_follower_number_last, gamer_friend_number_last)+4)):
                worksheet.update_values("B{}".format(i), [["", "", "", "", "", "", "", "", "", "", "", "", "", ""]]) #寫入""
            ...
            code_section_6_state="〇"
        except:
            code_section_6_state="✕"
        ...
        try:
            ...
            if str(start_time).split(" ")[0] == str(worksheet.get_value("E{}".format(number+2))): #start_time的日期等於("D{}".format(number+1))的字串
                ...
                worksheet.update_value("C{}".format(number+3), int(worksheet.get_value("C{}".format(number+2)))+1) #取得前一格的數字後加一再寫入
            else:
                worksheet.update_value("B{}".format(number+3), date_number+1) #寫入天數+1
                worksheet.update_value("C{}".format(number+3), "1") #寫入數字1
            ...
            worksheet.update_value("I{}".format(number+3), code_section_1_state) #寫入程式運作狀態 #裝置基本資訊取得
            worksheet.update_value("J{}".format(number+3), code_section_2_state) #寫入程式運作狀態 #追蹤者、好友和流量統計數取得
            worksheet.update_value("K{}".format(number+3), code_section_3_state) #寫入程式運作狀態 #追蹤者名單取得
            worksheet.update_value("L{}".format(number+3), code_section_4_state) #寫入程式運作狀態 #好友名單取得
            worksheet.update_value("M{}".format(number+3), code_section_5_state) #寫入程式運作狀態 #人氣記錄表寫入
            worksheet.update_value("N{}".format(number+3), code_section_6_state) #寫入程式運作狀態 #名單寫入
            ...
            code_section_7_state="〇"
        except:
            code_section_7_state="✕" 
            worksheet.update_value("A{}".format(number+3), runtime+1) #寫入本次的運作次數
            if str(start_time).split(" ")[0] == str(worksheet.get_value("E{}".format(number+2))): #start_time的日期等於("D{}".format(number+1))的字串
                worksheet.update_value("B{}".format(number+3), date_number) #寫入同樣天數
                worksheet.update_value("C{}".format(number+3), int(worksheet.get_value("C{}".format(number+2)))+1) #取得前一格的數字後加一再寫入
            ...
            worksheet.update_value("E{}".format(number+3), str(start_time).split(" ")[0].split("+")[0]) #寫入現在的時間
            worksheet.update_value("F{}".format(number+3), str(start_time).split(" ")[1].split("+")[0]) #寫入程式開始的時間
    worksheet.update_value("O{}".format(number+3), code_section_7_state) #寫入程式運作狀態 #系統訊息寫入
