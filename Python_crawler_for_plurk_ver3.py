...
def get_followday(number, start_time, i, state, startfollowdate, followday, string, endfollowdate): #取得追蹤天數
    if state == "old": #如果狀態為old(老追蹤者)
        try:
            ...
            if str(start_time).split(" ")[0] == str(open_googlesheets.worksheet('id', 1068388458).get_value("E{}".format(number+2))): #start_time的日期等於噗浪追蹤名單("D{}".format(number+1))的字串
    ...
    for i in range(len(plurk_data_get["other"]["plurk_fan_accountlist"])): #依照順序數字建立字典，再將資訊分別塞進去
        ...
        if plurk_fan_statuscode == 200: #如果網路狀態碼正常
            ...
            try: #測試第一個script的起始順序
                down = down+1
                jscode_1st = plurk_fan_sourcecode.xpath("/html/body/script[{}]/text()".format(down))[0] #取得text()在網頁程式碼的位置(Xpath表達式)
            except:
                down = down+1
                jscode_1st = plurk_fan_sourcecode.xpath("/html/body/script[{}]/text()".format(down))[0] #取得text()在網頁程式碼的位置(Xpath表達式)
            down = down+2
            xpath_on_web = "/html/body/script[{}]/text()".format(down) #指定text()在網頁程式碼的位置(Xpath表達式)
...
try:
    ...
    for i in range(len(plurk_data_get["other"]["plurk_friend_accountlist"])): #依照順序數字建立字典，再將資訊分別塞進去
        ...
        if plurk_friend_statuscode == 200:
            ...
            try: #測試第一個script的起始順序
                down = down+1
                jscode_1st = plurk_friend_sourcecode.xpath("/html/body/script[{}]/text()".format(down))[0] #取得text()在網頁程式碼的位置(Xpath表達式)
            except:
                down = down+1
                jscode_1st = plurk_friend_sourcecode.xpath("/html/body/script[{}]/text()".format(down))[0] #取得text()在網頁程式碼的位置(Xpath表達式)
            down = down+2
...
if open_googlesheets_state == 1:
    try:
        ...
        try:
            runtime = int(worksheet.get_value("A{}".format(number+2))) #獲取運作次數，number+2為END前一格的位置
        except:
            runtime = 0 #獲取運作次數0次
    ...
    if basic_state == 1:
        try:
            ...
            if str(datetime.strptime(worksheet.get_value("A{}".format(writesit-1))+worksheet.get_value("C{}".format(writesit-1)), "%Y年%m月%d日")) != str(datetime.strptime(ad_year_today+date_today, "%Y年%m月%d日")):
        ...
        try:
            #寫入試算表3："噗浪追蹤名單"
            worksheet = open_googlesheets.worksheet('id', 1068388458) #定位試算表位置為"噗浪追蹤名單"
            ...
            for i in range(len(plurk_data_get["fan"])): #範圍為fan鍵的值(字典)內所含有的數量
                ...
                try:
                    if plurk_data_get["fan"][i]["account"] in plurk_data_last["other"]["plurk_fan_accountlist"]: #如果從噗浪取得的粉絲account的值有在串列裡
                        plurk_data_new["fan"][plurk_fan_number_new]["state"] = "old" #標記狀態為old
            ...
            for i in range(len(plurk_data_get["fan"])): #範圍為fan鍵的值(字典)內所含有的數量
                ...
                try:
                    if plurk_data_get["fan"][i]["account"] not in plurk_data_last["other"]["plurk_fan_accountlist"]: #如果從噗浪取得的粉絲account的值沒有在串列裡
                        plurk_data_new["fan"][plurk_fan_number_new]["state"] = "new" #標記狀態為new
                        plurk_data_new["fan"][plurk_fan_number_new]["account"] = plurk_data_get["fan"][i]["account"] #將從噗浪取得的粉絲account、displayname、建立日和最後上線日記錄進plurk_data_new
                        plurk_data_new["fan"][plurk_fan_number_new]["displayname"] = plurk_data_get["fan"][i]["displayname"]
                        plurk_data_new["fan"][plurk_fan_number_new]["regdate"] = plurk_data_get["fan"][i]["regdate"]
                        plurk_data_new["fan"][plurk_fan_number_new]["lastondate"] = plurk_data_get["fan"][i]["lastondate"]
            ...
            for i in range(len(plurk_data_get["fan"])): #範圍為fan鍵的值(字典)內所含有的數量
                ...
                try:
                    if plurk_data_last["fan"][i]["account"] not in plurk_data_get["other"]["plurk_fan_accountlist"]: #如果從試算表取得的粉絲account的值沒有在串列裡
                        plurk_data_new["fan"][plurk_fan_number_new]["state"] = "leave" #標記狀態為leave
                        plurk_data_new["fan"][plurk_fan_number_new]["account"] = plurk_data_get["fan"][i]["account"] #將從噗浪取得的粉絲account、displayname、建立日和最後上線日記錄進plurk_data_new
                        plurk_data_new["fan"][plurk_fan_number_new]["displayname"] = plurk_data_get["fan"][i]["displayname"]
                        plurk_data_new["fan"][plurk_fan_number_new]["regdate"] = plurk_data_get["fan"][i]["regdate"]
                        plurk_data_new["fan"][plurk_fan_number_new]["lastondate"] = plurk_data_get["fan"][i]["lastondate"]
                        ...
                        plurk_data_new["fan"][plurk_fan_number_new]["followday"] = get_followday(number, start_time, i, plurk_data_new["fan"][plurk_fan_number_new]["state"], plurk_data_new["fan"][plurk_fan_number_new]["startfollowdate"], plurk_data_last["fan"][plurk_data_last["other"]["plurk_fan_accountlist"].index(plurk_data_new["fan"][plurk_fan_number_new]["account"])]["followday"], plurk_fan_string, plurk_data_new["fan"][plurk_fan_number_new]["endfollowdate"]) #傳入副程式start_time, i, state, startfollowdate, followday, string, endfollowdate這些參數，取得追蹤天數
                        plurk_fan_number_new = plurk_fan_number_new+1
                    else:
            ...
            for i in range(len(plurk_data_get["friend"])): #範圍為friend鍵的值(字典)內所含有的數量
                ...
                try:
                    if plurk_data_get["friend"][i]["account"] in plurk_data_last["other"]["plurk_friend_accountlist"]: #如果從噗浪取得的朋友account的值有在串列裡
                        plurk_data_new["friend"][plurk_friend_number_new]["state"] = "old" #標記狀態為old
            ...
            for i in range(len(plurk_data_get["friend"])): #範圍為friend鍵的值(字典)內所含有的數量
                ...
                try:
                    if plurk_data_get["friend"][i]["account"] not in plurk_data_last["other"]["plurk_friend_accountlist"]: #如果從噗浪取得的朋友account的值沒有在串列裡
                        plurk_data_new["friend"][plurk_friend_number_new]["state"] = "new" #標記狀態為new
                        plurk_data_new["friend"][plurk_friend_number_new]["account"] = plurk_data_get["friend"][i]["account"] #將從噗浪取得的朋友account、displayname、建立日和最後上線日記錄進plurk_data_new
            ...
            for i in range(len(plurk_data_get["friend"])): #範圍為friend鍵的值(字典)內所含有的數量
                ...
                try:
                    if plurk_data_last["friend"][i]["account"] not in plurk_data_get["other"]["plurk_friend_accountlist"]: #如果從試算表取得的朋友account的值沒有在串列裡
                        plurk_data_new["friend"][plurk_friend_number_new]["state"] = "leave" #標記狀態為leave
                        plurk_data_new["friend"][plurk_friend_number_new]["account"] = plurk_data_get["friend"][i]["account"] #將從噗浪取得的朋友account、displayname、建立日和最後上線日記錄進plurk_data_new
                        plurk_data_new["friend"][plurk_friend_number_new]["displayname"] = plurk_data_get["friend"][i]["displayname"]
                        plurk_data_new["friend"][plurk_friend_number_new]["regdate"] = plurk_data_get["friend"][i]["regdate"]
                        plurk_data_new["friend"][plurk_friend_number_new]["lastondate"] = plurk_data_get["friend"][i]["lastondate"]
                        ...
                        plurk_data_new["friend"][plurk_friend_number_new]["followday"] = get_followday(number, start_time, i, plurk_data_new["friend"][plurk_friend_number_new]["state"], plurk_data_new["friend"][plurk_friend_number_new]["startfollowdate"], plurk_data_last["friend"][plurk_data_last["other"]["plurk_friend_accountlist"].index(plurk_data_new["friend"][plurk_friend_number_new]["account"])]["followday"], plurk_friend_string, plurk_data_new["friend"][plurk_friend_number_new]["endfollowdate"]) #傳入副程式start_time, i, state, startfollowdate, followday, string, endfollowdate這些參數，取得追蹤天數
                        plurk_friend_number_new = plurk_friend_number_new+1
                    else:
                        del plurk_data_new["friend"][plurk_friend_number_new] #如果從噗浪取得的朋友account的值沒有在串列裡，刪除以plurk_friend_number_new為名的鍵
                except:
            ...
            for i in range(3, (max(plurk_fan_number_last, plurk_friend_number_last)+4)):
                worksheet.update_values("B{}".format(i), [["", "", "", "", "", "", "", "", "", "", "", "", "", ""]]) #寫入「完全空白」
        ...
        try:
            ...
            if str(start_time).split(" ")[0] == str(worksheet.get_value("E{}".format(number+2))): #start_time的日期等於("E{}".format(number+1))的字串
